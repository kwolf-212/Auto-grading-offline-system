# exam_generator_app.py (PDF preview가 완전히 동작하는 수정 코드)
...
try:
    ...
    print("PDF preview will show placeholder images.")
...
class PDFPreviewWidget(QWidget):
    ...
    def update_preview(self):
        ...
        if not PYMUPDF_AVAILABLE:
            # Fallback: show placeholder with file info
        ...
        try:
            ...
            if self.total_pages > 0:
                ...
                # Render page to image (zoom for higher quality)
                zoom_matrix = fitz.Matrix(1.5, 1.5)  # 150% for better quality
                ...
                self.status_label.setText(f"Preview loaded | Page {self.current_page + 1}/{self.total_pages} | Zoom: {self.zoom_slider.value()}%")
            else:
    ...
    def load_pdf(self, pdf_path):
        """Load a new PDF file and reset to first page"""
...
class GeneratorApp(QMainWindow):
    def __init__(self):
        ...
        # Initial preview after UI loads
    ...
    def init_ui(self):
        ...
        left_layout.addWidget(exam_info_group)
        ...
        # Connect refresh button to our generate method
        # 안전하게 disconnect 시도
        try:
            self.pdf_preview.refresh_btn.clicked.disconnect()
        except TypeError:
            # 연결된 시그널이 없으면 무시
            pass
    ...
    def _generate_pdf_to_file(self, file_path, is_preview=False):
        ...
        for q in self.questions:
            ...
            text_line = f"Q{q['id']}. {q_text_display} ({q['score']} pts)"
